...
def iterative_reconstruction(Niter, L, nc, zlow, zhigh, tracer, matter, observer, redshift_pos='PositionRSD',
    ...
    for i in range(Niter):
        ...
        if plot_iterations:
            k = r.power['k']
            ax.loglog(k, Pk1, ':', color='gray', label='Iteration {:1n}'.format(i))
            ax.loglog(k, Pk2, color='green', label='Iteration {:1n}'.format(i+1))
            ax.legend()
    ...
    # Final reconstruction estimate: s(zhigh) = s(zlow) - (D(zlow) - D(zhigh))Psi(q) - (vr(zlow) - vr(r(zhigh))
    q = tracer['PositionQ'].compute()
    delta = tracer.to_mesh(resampler='cic', position='PositionQ', interlaced=True, compensated=True)
