...
qs = halos['PositionQS'].compute()
deltaQS = halos.to_mesh(resampler='cic', position='PositionQS', interlaced=True, compensated=True)

psi = compute_zeld(Length, Nc, deltaQS.apply(divide_bias, mode='real', kind='index')\
    .apply(filters.Gaussian(r=r_s).filter, mode='complex', kind='wavenumber').compute(mode='real'))
psi_g = field_interpolation(Length, Nc, psi, qs)
vr = compute_vr(psi_g, qs, observer, zinit)
halos['PositionQ'] = qs - vr
...
# print(mat[:2])
with open('pairedsim'+str(paired)+'_cat_post_z3_vrcorrected.dat', 'wb') as ff:
    for line in mat:
        np.savetxt(ff, line, fmt='%.5f')

# halos.save('Halos_paired'+str(paired)+'_vrcorrected.bigfile')